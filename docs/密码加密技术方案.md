...
本方案提供一套完整的配置文件密码加密技术，实现敏感信息的加密存储和解密使用，确保生产环境中敏感信息的安全性。
...
- **密钥存储**
  - 开发环境: 环境变量或本地密钥文件
  - 生产环境: 专业密钥管理系统 (KMS/HSM)

### 2. 实现架构

```
配置文件 → 加密存储 → 运行时解密 → 应用使用
    ↓           ↓          ↓         ↓
明文配置 → AES加密 → 密文配置 → 内存解密 → 业务逻辑
```

### 3. 核心组件

#### 3.1 加密工具类 (EncryptUtil)
- 字符串加密/解密
- 文件加密/解密
- 密钥派生和管理

#### 3.2 配置处理器 (EncryptedPropertyProcessor)
- Spring Boot配置属性处理器
- 自动解密配置值
- 支持嵌套配置解密

#### 3.3 密钥管理器 (KeyManager)
- 密钥生成和存储
- 密钥轮换支持
- 多环境密钥管理
...
📊 **完整流程图请点击查看**: [密码加密流程图.svg](./images/密码加密流程图.svg)

![加密流程总览](password_encryption_flow_diagram.png)

**🔗 在线查看完整流程图**: 
- 📁 文件路径: `docs/images/密码加密流程图.svg`
- 🌐 在浏览器中打开即可查看矢量流程图

> **提示**: SVG版本包含彩色流程图、矢量图形和详细的技术说明，比纯ASCII图更直观易懂！

> **🚀 SVG版本包含以下优势**:
> - 🎨 **彩色流程图**: 使用颜色编码区分不同阶段的处理过程
> - 📐 **矢量图形**: 无损缩放，适合各种尺寸的显示设备
> - 🔧 **技术细节**: 包含参数配置、性能优化和最佳实践指南
> - ⚡ **性能优化**: 轻量级文件，加载速度快

### Spring Boot配置解析流程

🔗 **详细流程图请点击查看**: [密码加密流程图.svg](./images/密码加密流程图.svg#spring-boot配置解析流程)

### 配置文件生命周期管理

🔗 **详细流程图请点击查看**: [密码加密流程图.svg](./images/密码加密流程图.svg#配置文件生命周期管理)
...
app:
  ...
  # 加密后的敏感信息
  encrypted:
    database-password: ENC(AES加密后的数据库密码)
    redis-password: ENC(AES加密后的Redis密码)
    jwt-secret: ENC(AES加密后的JWT密钥)
    api-key: ENC(AES加密后的API密钥)
...
### 1. 优势
- **强加密**: AES-256-GCM提供军事级安全性
- **认证加密**: GCM模式提供认证功能，防止篡改
- **密钥派生**: PBKDF2防止暴力破解
- **自动解密**: 对应用透明，自动处理加密/解密
- **灵活配置**: 支持多种密钥管理方式

### 2. 风险和缓解
- **密钥泄露**: 使用专业KMS系统，定期轮换密钥
- **内存泄露**: 解密后立即使用，及时清理内存
- **配置泄露**: 确保配置文件访问权限，使用环境变量

### 3. 性能影响
- **启动时间**: 首次解密需要时间，后续使用缓存
- **内存使用**: 解密后缓存在内存中
- **计算开销**: AES-GCM加解密性能优秀
...
### 1. 解密操作日志
```java
// 在EncryptedPropertyProcessor中添加日志
...
本方案提供了完整的配置文件密码加密技术实现，具有以下特点：

1. **安全可靠**: 采用AES-256-GCM军事级加密算法
2. **易于集成**: 与Spring Boot无缝集成，对业务代码透明
3. **灵活配置**: 支持多种密钥管理方式
4. **性能优秀**: 使用缓存机制，最小化性能影响
5. **生产就绪**: 支持多种部署环境和密钥管理策略

建议在生产环境中使用时，配合专业的密钥管理系统(KMS)一起使用，以获得最佳的安全性保障。